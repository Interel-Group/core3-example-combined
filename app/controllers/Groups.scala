...
class Groups @Inject()(engine: WorkflowEngine, cache: SyncCacheApi, db: DatabaseAbstractionLayer)
  ...
  def delete() = AuthorizedAction(
    ...
    okHandler = { (request, user) =>
      ...
      Groups.Forms.delete.bindFromRequest.fold(
        ...
        params =>
          for {
            result <- engine.executeWorkflow(SystemDeleteGroup.name, params.asJson, user)
          } yield {
            if (result.wasSuccessful) {
