...
import core3.workflows.{NoWorkflowParameters, WorkflowBase, WorkflowEngine}
...
class Users @Inject()(engine: WorkflowEngine, cache: SyncCacheApi, db: DatabaseAbstractionLayer, workflows: Vector[WorkflowBase])
  ...
  private val authConfig = StaticConfig.get.getConfig("security.authentication.clients.LocalCombinedExample")
  ...
  def delete() = AuthorizedAction(
    ...
    okHandler = { (request, user) =>
      ...
      Users.Forms.delete.bindFromRequest.fold(
        ...
        params =>
          for {
            result <- engine.executeWorkflow(SystemDeleteLocalUser.name, params.asJson, user)
          } yield {
            if (result.wasSuccessful) {
