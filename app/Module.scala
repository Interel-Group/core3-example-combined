...
class Module extends AbstractModule {
  ...
  def provideBootstrap(system: ActorSystem, db: DatabaseAbstractionLayer, workflows: Vector[WorkflowBase])(implicit ec: ExecutionContext): Bootstrap = {
    val authConfig = StaticConfig.get.getConfig("security.authentication.clients.LocalCombinedExample")
  ...
  def provideLocalConsole(manager: ComponentManager, engine: WorkflowEngine, db: DatabaseAbstractionLayer)(implicit ec: ExecutionContext): Option[LocalConsole] = {
    Option(System.getProperty("c3ec.console")) match {
      case Some("enabled") =>
        val appVendor: String = core3_example_combined.BuildInfo.organization
        val appName: String = core3_example_combined.BuildInfo.name
        val appVersion: String = core3_example_combined.BuildInfo.version
